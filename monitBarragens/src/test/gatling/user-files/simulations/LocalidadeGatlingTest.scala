...
/**
 * Performance test for the Localidade entity.
 */
class LocalidadeGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the Localidade entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all localidades")
            .get("/services/monitbarragens/api/localidades")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new localidade")
            .post("/services/monitbarragens/api/localidades")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "id":null
                , "nome":"SAMPLE_TEXT"
            ...
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_localidade_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created localidade")
                .get("/services/monitbarragens${new_localidade_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created localidade")
            .delete("/services/monitbarragens${new_localidade_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
