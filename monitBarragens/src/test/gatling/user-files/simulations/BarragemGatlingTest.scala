...
/**
 * Performance test for the Barragem entity.
 */
class BarragemGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the Barragem entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all barragems")
            .get("/services/monitbarragens/api/barragems")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new barragem")
            .post("/services/monitbarragens/api/barragems")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "id":null
                }""")).asJson
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_barragem_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created barragem")
                .get("/services/monitbarragens${new_barragem_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created barragem")
            .delete("/services/monitbarragens${new_barragem_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
