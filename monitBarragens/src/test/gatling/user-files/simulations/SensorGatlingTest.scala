...
/**
 * Performance test for the Sensor entity.
 */
class SensorGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the Sensor entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all sensors")
            .get("/services/monitbarragens/api/sensors")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new sensor")
            .post("/services/monitbarragens/api/sensors")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "id":null
                , "numero":"0"
            ...
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_sensor_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created sensor")
                .get("/services/monitbarragens${new_sensor_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created sensor")
            .delete("/services/monitbarragens${new_sensor_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
