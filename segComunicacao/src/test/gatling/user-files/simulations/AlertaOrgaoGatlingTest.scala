...
/**
 * Performance test for the AlertaOrgao entity.
 */
class AlertaOrgaoGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the AlertaOrgao entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all alertaOrgaos")
            .get("/services/segcomunicacao/api/alerta-orgaos")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new alertaOrgao")
            .post("/services/segcomunicacao/api/alerta-orgaos")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "id":null
                }""")).asJson
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_alertaOrgao_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created alertaOrgao")
                .get("/services/segcomunicacao${new_alertaOrgao_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created alertaOrgao")
            .delete("/services/segcomunicacao${new_alertaOrgao_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
