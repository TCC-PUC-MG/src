...
/**
 * Performance test for the NivelSituacao entity.
 */
class NivelSituacaoGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the NivelSituacao entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all nivelSituacaos")
            .get("/services/segcomunicacao/api/nivel-situacaos")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new nivelSituacao")
            .post("/services/segcomunicacao/api/nivel-situacaos")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "id":null
            ...
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_nivelSituacao_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created nivelSituacao")
                .get("/services/segcomunicacao${new_nivelSituacao_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created nivelSituacao")
            .delete("/services/segcomunicacao${new_nivelSituacao_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
