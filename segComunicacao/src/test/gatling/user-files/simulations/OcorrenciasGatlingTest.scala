...
/**
 * Performance test for the Ocorrencias entity.
 */
class OcorrenciasGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the Ocorrencias entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all ocorrencias")
            .get("/services/segcomunicacao/api/ocorrencias")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new ocorrencias")
            .post("/services/segcomunicacao/api/ocorrencias")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "id":null
            ...
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_ocorrencias_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created ocorrencias")
                .get("/services/segcomunicacao${new_ocorrencias_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created ocorrencias")
            .delete("/services/segcomunicacao${new_ocorrencias_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
