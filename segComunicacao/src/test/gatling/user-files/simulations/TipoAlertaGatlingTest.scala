...
/**
 * Performance test for the TipoAlerta entity.
 */
class TipoAlertaGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the TipoAlerta entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all tipoAlertas")
            .get("/services/segcomunicacao/api/tipo-alertas")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new tipoAlerta")
            .post("/services/segcomunicacao/api/tipo-alertas")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "id":null
            ...
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_tipoAlerta_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created tipoAlerta")
                .get("/services/segcomunicacao${new_tipoAlerta_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created tipoAlerta")
            .delete("/services/segcomunicacao${new_tipoAlerta_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
