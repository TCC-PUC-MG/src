...
/**
 * Performance test for the OrgaoGoverno entity.
 */
class OrgaoGovernoGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the OrgaoGoverno entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all orgaoGovernos")
            .get("/services/segcomunicacao/api/orgao-governos")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new orgaoGoverno")
            .post("/services/segcomunicacao/api/orgao-governos")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "id":null
                , "descricao":"SAMPLE_TEXT"
                , "pessoaContato":"SAMPLE_TEXT"
                , "telefone":"SAMPLE_TEXT"
                }""")).asJson
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_orgaoGoverno_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created orgaoGoverno")
                .get("/services/segcomunicacao${new_orgaoGoverno_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created orgaoGoverno")
            .delete("/services/segcomunicacao${new_orgaoGoverno_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
